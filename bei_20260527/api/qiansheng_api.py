...
class BaseAPIClient:
    ...
    def _clean_data(self, data: Any) -> Dict[str, Any]:
        """
        内部辅助方法：清洗数据
        1. 将 dataclass 转为字典
        2. 过滤掉值为 None 的参数
...
class TaskGroupItem:
    ...
    studySection: Optional[str] = None
    totalCount: Optional[int] = None
    doneCount: Optional[int] = None
    ...
    def get_item_by_list(self,
        ...
        if not is_cache_valid:
            ...
            if task_detail_query is None:
                ...
                if task_group_item.subjectId is not None:
                    task_detail_query.subjectId = task_group_item.subjectId
                if task_group_item.taskType is not None:
...
def get_taskgroup(subjectId, task_type,status,page_size):
    task_group_service = TaskGroupService()
    query_params = TaskGroupQuery(
        subjectId=subjectId,
        taskType=task_type,
...
#     try:
#         response = requests.get(CONFIG_BASE_URL, params=params, timeout=30)
#         response.raise_for_status()
#         return response.json()
#     except requests.exceptions.RequestException as e:
#         return {
...
def get_taskgroup_list(taskgroup_id, task_type, subjectid):
    taskgroup_info = TaskGroupItem(id=taskgroup_id)
    task_detail_service = TaskDetailService()
    res = None
    index = 0
    total = 1
    all_items = []
    task_detail_query = TaskDetailQuery(
        taskGroupId=taskgroup_info.id,
        subjectId=subjectid,
        taskType=task_type,
    )
    while index < total:
        item, res, total = \
            task_detail_service.get_item_by_list(taskgroup_info,
                                                 index,
                                                 last_query_res=res,
    ...
    # print(f"全量数据轮询处理完毕！共获取 {len(all_items)} 条任务")
    return all_items
...
if __name__ == '__main__':
    ...
    subjectId = 23
    task_type = 2
    taskgroup = get_taskgroup(subjectId=subjectId, task_type=task_type,status = 0,page_size=100)
    taskgroup.rows.to_dict()
    print(f"taskgroup: {taskgroup}")
    a = get_taskgroup_list(taskgroup_id = 8824, task_type =2 ,subjectid = 23)
